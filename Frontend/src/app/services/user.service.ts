    ...
    .pipe(
      map(response => {
        if (response.token) {
          ...
          // Sprawdź typ konta i ustaw isModeration na true, jeśli accountType > 0
          this.getAccountByEmail(email).subscribe(account => {
            console.log('Account details:', account); // Logowanie szczegółów konta
            const isMod = account.accountType > 0;  // Zmiana z account_type na accountType
            console.log('isModeration:', isMod); // Logowanie wartości isModeration
